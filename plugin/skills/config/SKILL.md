---
description: "Customize the Coach Claw display — statusline variant, rank-name theme, ELO range. Usage: /coach-claw:config [show|preview|statusline <variant>|theme <name>|elo <min> <max>|reset]"
---
...
The argument after `/coach-claw:config` is one of:
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/run.sh - <<PY
import os, sys
sys.path.insert(0, os.environ.get("CLAUDE_PLUGIN_ROOT", "") + "/bin")
...
Delegates to `configure.py preview` (via `run.sh`, which routes through
the plugin's PyYAML venv) so the slash command and the `npx coach-claw
config preview` terminal command run literally the same code — output
is byte-identical, no drift surface.
...
```bash
VARIANT="$1"  # the second word from the user's invocation
${CLAUDE_PLUGIN_ROOT}/bin/run.sh - "$VARIANT" <<PY
import os, sys
sys.path.insert(0, os.environ.get("CLAUDE_PLUGIN_ROOT", "") + "/bin")
...
```bash
THEME="$1"
${CLAUDE_PLUGIN_ROOT}/bin/run.sh - "$THEME" <<PY
import os, sys
sys.path.insert(0, os.environ.get("CLAUDE_PLUGIN_ROOT", "") + "/bin")
...
```bash
MIN="$1"; MAX="$2"
${CLAUDE_PLUGIN_ROOT}/bin/run.sh - "$MIN" "$MAX" <<PY
import os, sys
sys.path.insert(0, os.environ.get("CLAUDE_PLUGIN_ROOT", "") + "/bin")
...
try:
    ...
    print("usage: /coach-claw:config elo <min> <max>"); sys.exit(1)
if not (0 < emin < emax):
...
- For `elo`: the threshold curve is hardcoded; only the ELO
  ...
  thresholds, point at `${CLAUDE_PLUGIN_ROOT}/bin/stats.py:_build_level_ladder`
  and warn that doing so retroactively shifts existing user levels.
